...
import application.AuthorFeedback;
import application.Message;
import application.ModerationRequest;
import application.Review;
import application.RoleRemovalRequest;
import application.ScoreParam;
...
//			statement.execute("DROP ALL OBJECTS");
	...
	private void createTables() throws SQLException {
        ...
        // Create a moderation_requests table:
        String moderationTable = "CREATE TABLE IF NOT EXISTS moderation_requests ("
        		+ "  id INT AUTO_INCREMENT PRIMARY KEY, "
        		+ "  content_type VARCHAR(20), "     // "question", "answer", or "comment"
        		+ "  content_id   INT, "             // the PK of the flagged content
        		+ "  instructor   VARCHAR(255), "    // who requested review
        ...
        // Create a parameters table to hold instructor-defined weights for reviewers
        String reviewerScoreTable = "CREATE TABLE IF NOT EXISTS reviewer_score_parameters ("
        		+ "id         INT AUTO_INCREMENT PRIMARY KEY, "
        		+ "domain     VARCHAR(50) NOT NULL, "
        		+ "weight     DECIMAL(5,2) NOT NULL, "
        		+ "updated_by VARCHAR(255) NOT NULL, "
        		+ "updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)";
        statement.execute(reviewerScoreTable);
        
        String countSql = "SELECT COUNT(*) AS cnt FROM reviewer_score_parameters";
        try (ResultSet rs = statement.executeQuery(countSql)) {
            if (rs.next() && rs.getInt("cnt") == 0) {
                // first‐time setup: insert default domains with equal weights
                String insertDefaults =
                   "INSERT INTO reviewer_score_parameters (domain, weight, updated_by) VALUES "
                 + "('thoroughness', 0.33, 'system'), "
                 + "('clarity',      0.33, 'system'), "
                 + "('timeliness',   0.34, 'system')";
                statement.execute(insertDefaults);
            }
        }
        
        String reviewerProfileTable = "CREATE TABLE IF NOT EXISTS reviewer_profiles ("
        		+ "username   VARCHAR(255) PRIMARY KEY, "
        		+ "experience TEXT, "
        		+ "updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)";
        ...
        String countSql1 = 
          "SELECT COUNT(*) AS cnt FROM reviewer_profiles";
        try (ResultSet rs = statement.executeQuery(countSql1)) {
            if (rs.next() && rs.getInt("cnt") == 0) {
                // First‐run: insert an empty profile for every existing reviewer account
                String seedSql =
                  "INSERT INTO reviewer_profiles (username, experience) " +
                  "SELECT userName, '' FROM cse360users WHERE role LIKE '%reviewer%'";
        ...
        String authorFeedbackTable = "CREATE TABLE IF NOT EXISTS author_feedback ("
        		+ "id             INT AUTO_INCREMENT PRIMARY KEY, "
        		...
        		+ "FOREIGN KEY (reviewer) REFERENCES cse360users(userName), "
        		+ "FOREIGN KEY (author)   REFERENCES cse360users(userName))";
        statement.execute(authorFeedbackTable);
	...
	// Method to fetch all role removal requests
	 ...
	 //
	 // Moderation Request Functionality
	 //
	    
	    // Submit a new moderation request
	    public void addModerationRequest(String contentType, int contentId, String instructor, String reason) throws SQLException {
	        String sql = "INSERT INTO moderation_requests (content_type, content_id, instructor, reason) VALUES (?, ?, ?, ?)";
	    ...
	    // Close a request
	    public void closeModerationRequest(int requestId) throws SQLException {
	        String sql = "UPDATE moderation_requests SET status='CLOSED', closed_time=CURRENT_TIMESTAMP WHERE id=?";
	        try (PreparedStatement ps = connection.prepareStatement(sql)) {
	    ...
	    // Fetch open requests
	    public List<ModerationRequest> getOpenModerationRequests() throws SQLException {
	        List<ModerationRequest> list = new ArrayList<>();
	        String sql = "SELECT * FROM moderation_requests WHERE status='OPEN' ORDER BY request_time DESC";
	        try (Statement st = connection.createStatement();
	             ResultSet rs = st.executeQuery(sql)) {
	            while (rs.next()) {
	                list.add(new ModerationRequest(
	                  rs.getInt("id"),
	    ...
	    public List<ScoreParam> getScoreParameters() throws SQLException {
	        List<ScoreParam> list = new ArrayList<>();
	        String sql = "SELECT domain, weight FROM reviewer_score_parameters";
	        try (Statement st = connection.createStatement();
	             ResultSet rs = st.executeQuery(sql)) {
	            while(rs.next()) {
	                list.add(new ScoreParam(rs.getString("domain"),
	                                        rs.getDouble("weight")));
	            }
	        }
	        return list;
	    ...
	    public void updateScoreParameter(String domain, double weight, String instructor) throws SQLException {
	        String sql = "MERGE INTO reviewer_score_parameters KEY(domain) " +
	                     "VALUES (?, ?, ?)";
	        try (PreparedStatement ps = connection.prepareStatement(sql)) {
	    ...
	    public void addAuthorFeedback(String reviewer, String author, int rating, String comment) throws SQLException {
	        String sql = "INSERT INTO author_feedback (reviewer, author, rating, comment) VALUES (?, ?, ?, ?)";
	        try (PreparedStatement ps = connection.prepareStatement(sql)) {
	    ...
	    // Fetch the average “usefulness” rating that authors have given this reviewer.
	    public double getAuthorFeedbackForReviewer(String reviewer) throws SQLException {
	        String sql = "SELECT AVG(rating) AS avgRating "
	                   + "FROM author_feedback WHERE reviewer = ?";
	        try (PreparedStatement ps = connection.prepareStatement(sql)) {
	            ps.setString(1, reviewer);
	        ...
	        return 0.0;  // or some default if no ratings exist
	    }
	    
	    // Returns a list of all users with the 'reviewer' role.
	    public List<String> getAllReviewers() throws SQLException {
	        List<String> out = new ArrayList<>();
	        String sql = "SELECT userName FROM cse360users WHERE role LIKE '%reviewer%'";
	        try (Statement st = connection.createStatement();
	             ResultSet rs = st.executeQuery(sql)) {
	            while (rs.next()) {
	    ...
	    // Fetch a reviewer’s experience text
	    public String getReviewerExperience(String username) throws SQLException {
	        String sql = "SELECT experience FROM reviewer_profiles WHERE username = ?";
	        try (PreparedStatement ps = connection.prepareStatement(sql)) {
	            ps.setString(1, username);
	            try (ResultSet rs = ps.executeQuery()) {
	                return rs.next() ? rs.getString("experience") : "";
	            }
	        }
	    }

	    // Update a reviewer’s experience
	    public void updateReviewerExperience(String username, String experience) throws SQLException {
	        String sql = "UPDATE reviewer_profiles SET experience = ?, updated_at = CURRENT_TIMESTAMP WHERE username = ?";
	        try (PreparedStatement ps = connection.prepareStatement(sql)) {
	 ...
	 // List all reviews a reviewer has written (reuse your existing DAO)
	    public List<Review> getReviewsByReviewer(String reviewer) throws SQLException {
	        // You can simply query reviews WHERE reviewerName=? using your getReviewsForTarget
	        // Or write a dedicated query:
	    	QuestionsAnswersDatabase qaDb = new QuestionsAnswersDatabase();
	    	qaDb.connectToQuestionAnswerDatabase();
	        String sql = 
	          "SELECT id, targetID, targetType, reviewerName, reviewText, timestamp, "
	        + "thoroughness_rating, clarity_rating, timeliness_rating "
	            ...
	            try (ResultSet rs = ps.executeQuery()) {
	                ...
	                while (rs.next()) {
	                    out.add(new Review(
	                      ...
	                      rs.getTimestamp("timestamp"),
	                      (Integer)rs.getObject("thoroughness_rating"),
	                      (Integer)rs.getObject("clarity_rating"),
	                      (Integer)rs.getObject("timeliness_rating")
	                    ));
	                }
	                return out;
	            }
	    ...
	    // List all feedback (ratings/comments) given back to this reviewer
	    public List<AuthorFeedback> getAuthorFeedbackForReviewerDetailed(String reviewer) throws SQLException {
	        String sql =
	          "SELECT id, reviewer, author, rating, comment, feedback_time "
	        + "FROM author_feedback WHERE reviewer = ?";
	        try (PreparedStatement ps = connection.prepareStatement(sql)) {
	            ps.setString(1, reviewer);
	            try (ResultSet rs = ps.executeQuery()) {
	                List<AuthorFeedback> out = new ArrayList<>();
	                while (rs.next()) {
	                    out.add(new AuthorFeedback(
	                        rs.getInt("id"),
