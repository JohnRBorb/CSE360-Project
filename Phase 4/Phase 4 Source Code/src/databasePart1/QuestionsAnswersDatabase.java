...
//			statement.execute("DROP ALL OBJECTS");
	...
	private void createTables() throws SQLException {
		...
		// Add a “reviewed” flag to questions, answers, comments for moderation:
	    statement.execute("ALTER TABLE questions ADD COLUMN IF NOT EXISTS reviewed BOOLEAN DEFAULT FALSE");
	    statement.execute("ALTER TABLE answers   ADD COLUMN IF NOT EXISTS reviewed BOOLEAN DEFAULT FALSE");
	    statement.execute("ALTER TABLE comments  ADD COLUMN IF NOT EXISTS reviewed BOOLEAN DEFAULT FALSE");
	    
	    statement.execute("ALTER TABLE reviews ADD COLUMN IF NOT EXISTS thoroughness_rating TINYINT");
	    statement.execute("ALTER TABLE reviews ADD COLUMN IF NOT EXISTS clarity_rating TINYINT");
	    statement.execute("ALTER TABLE reviews ADD COLUMN IF NOT EXISTS timeliness_rating TINYINT");
	}
	...
	private void createQuestionTable() throws SQLException {
		String questionTable = "CREATE TABLE IF NOT EXISTS questions ("
				...
				+ "  post_time TIMESTAMP DEFAULT CURRENT_TIMESTAMP"     // ← new column
				+ ")";
	...
	// Mark a piece of content reviewed
    public void markContentReviewed(String contentType, int contentId) throws SQLException {
        String table = switch(contentType.toLowerCase()) {
          case "question" -> "questions";
          case "answer"   -> "answers";
          case "comment"  -> "comments";
          default -> throw new IllegalArgumentException("Unknown type: " + contentType);
        };
        String sql = "UPDATE " + table + " SET reviewed=TRUE WHERE id=?";
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
    ...
    /** Update the three manual ratings on a review */
    public void updateReviewRatings(int reviewId, int thorough, int clarity, int timeliness) throws SQLException {
        String sql = "UPDATE reviews SET "
                   ...
                   + "WHERE id = ?";
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            ps.setInt(1, thorough);
            ps.setInt(2, clarity);
            ps.setInt(3, timeliness);
            ps.setInt(4, reviewId);
            ps.executeUpdate();
        }
    }
    
    // Returns the average rating of the given domain (thoroughness|clarity|timeliness)
    // for all reviews by this reviewer.
    public double getAvgRating(String reviewer, String domain) throws SQLException {
        // e.g. domain = "thoroughness" → column thoroughness_rating
        String column = domain + "_rating";
        String sql = "SELECT AVG(" + column + ") AS avgVal "
                   + "FROM reviews WHERE reviewerName = ?";
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            ps.setString(1, reviewer);
            try (ResultSet rs = ps.executeQuery()) {
                return rs.next() ? rs.getDouble("avgVal") : 0.0;
            }
        }
    }

    // Returns how many reviews this reviewer has received instructor ratings on.
    public int getRatedReviewCount(String reviewer) throws SQLException {
        String sql = 
          "SELECT COUNT(*) AS cnt "
        + "FROM reviews "
        + "WHERE reviewerName = ? "
        +   "AND thoroughness_rating IS NOT NULL";
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            ps.setString(1, reviewer);
            try (ResultSet rs = ps.executeQuery()) {
    ...
    // Returns the timestamp of the most recent review by this reviewer.
    public Timestamp getLastReviewTime(String reviewer) throws SQLException {
        String sql = 
        ...
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            ps.setString(1, reviewer);
            try (ResultSet rs = ps.executeQuery()) {
                return rs.next() ? rs.getTimestamp("lastTime") : null;
            }
        }
    }
